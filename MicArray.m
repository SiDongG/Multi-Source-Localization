...
% Date: Jan 23rd 2022
%% Initialization
d=0.2; %Microphone spacing in meters
rx=2;ry=3;rz=5;
...
sig_transmit = radiator(signal,[ang1_transmit ang2_transmit ang3_transmit ang4_transmit]);
...
sigr3 = collector1(sigp3,ang3_receive);
sigr4 = collector2(sigp4,ang4_receive);
...
xcor34_PHAT=abs(fftshift(ifft((fft(sigr3).*conj(fft(sigr4)))./(abs(fft(sigr3)).*abs(fft(sigr4))))));
figure()
hold on;box on
subplot(2,3,1)
...
subplot(2,3,4)
plot(1:length(xcor23_PHAT),(xcor23_PHAT))
subplot(2,3,5)
plot(1:length(xcor24_PHAT),(xcor24_PHAT))
subplot(2,3,6)
plot(1:length(xcor34_PHAT),(xcor34_PHAT))
...
Actual_Arrival(4)=(sqrt((rx-d)^2+(ry-d)^2+rz^2)/Propagation_Speed);

Actual_difference=zeros(1,6);
...
Actual_difference(4)=Actual_Arrival(2)-Actual_Arrival(3);
Actual_difference(5)=Actual_Arrival(2)-Actual_Arrival(4);
Actual_difference(6)=Actual_Arrival(3)-Actual_Arrival(4);

Peak_Value=zeros(1,6);
Estimated_difference=zeros(1,6);
for i=1:length(xcor12_PHAT)
    if xcor12_PHAT(i)>Peak_Value(1)
        Estimated_difference(1)=(i-1001);
        Peak_Value(1)=xcor12_PHAT(i);
    end
    if xcor13_PHAT(i)>Peak_Value(2)
        Estimated_difference(2)=(i-1001);
        Peak_Value(2)=xcor13_PHAT(i);
    end
    if xcor14_PHAT(i)>Peak_Value(3)
        Estimated_difference(3)=(i-1001);
        Peak_Value(3)=xcor14_PHAT(i);
    end
    if xcor23_PHAT(i)>Peak_Value(4)
        Estimated_difference(4)=(i-1001);
        Peak_Value(4)=xcor23_PHAT(i);
    end
    if xcor24_PHAT(i)>Peak_Value(5)
        Estimated_difference(5)=(i-1001);
        Peak_Value(5)=xcor24_PHAT(i);
    end
    if xcor34_PHAT(i)>Peak_Value(6)
        Estimated_difference(6)=(i-1001);
        Peak_Value(6)=xcor34_PHAT(i);
    end
end
